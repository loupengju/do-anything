...
import { Settings, Image as ImageIcon, LogOut, Home } from "lucide-react";
      ...
      <div className="flex h-full flex-col">
        <div className="flex h-16 items-center border-b border-primary/10 px-4">
          ...
          >
            ...
            <span className="bg-gradient-to-r from-primary to-blue-500 bg-clip-text text-transparent group-hover:from-blue-500 group-hover:to-primary transition-all duration-500">Do Anything</span>
          </Link>
        </div>
        <div className="flex-1 overflow-auto py-4 px-2">
          <div className="text-xs uppercase text-muted-foreground font-medium tracking-wider px-3 py-1.5">应用</div>
          <nav className="grid items-start px-2 text-sm font-medium gap-0.5 mt-0.5">
            <Link
              ...
              className={cn(
                ...
                pathname === "/" ? "bg-gradient-to-r from-purple-500/30 to-blue-500/30 text-zinc-900 dark:text-zinc-50 shadow-sm" : "text-zinc-500 dark:text-zinc-400"
              )}
            >
              <div className={cn(
        ...
        <div className="mt-auto p-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="w-full justify-start gap-2 hover:bg-primary/10 transition-all duration-300 rounded-lg py-2.5">
                <Avatar className="h-7 w-7 ring-1 ring-primary/30 transition-all duration-300 hover:ring-primary shadow-sm">
                  <AvatarImage src="" />
                  <AvatarFallback className="bg-gradient-to-r from-primary to-blue-500 text-white font-medium text-xs">用户</AvatarFallback>
                </Avatar>
                <div className="flex flex-col items-start">
                  <span className="font-medium text-xs">个人设置</span>
                  <span className="text-[10px] text-muted-foreground">管理您的账户</span>
                </div>
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 rounded-xl p-2 border-primary/10 shadow-lg animate-in fade-in-80 zoom-in-95">
